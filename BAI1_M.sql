...
-- 2 ) HIEN THI MA SINH VIEN , HO TEN NHUNG SINH VIEN THAM GIA DE TAI CO MA DT04
SELECT SINHVIEN.MASV,TENSV 
FROM SINHVIEN,DIEMSV
WHERE SINHVIEN.MASV = DIEMSV.MASV
AND MADT = 'DT04'

-- 3 ) TAO BO SUNG RANG BUOC RULE CHO COT DIEM CO GIA TRI 0 - 10
CREATE RULE R1 AS @CHECK BETWEEN 0 AND 10

------------------------------------------------
-- CREATE RULE R1 AS @CHECK < 10 AND @CHECK > 0
-- CREATE RULE R1 AS @CHECK IN ( 1,2,3,...,10)
GO
EXEC sp_bindrule 'R1','DIEMSV.DIEM'
-- CHU Y KHI NHAP DU LIEU DE KIEM TRA PHAI NAM TRONG BANG MA NO THAM CHIEU DEN

EXEC sp_unbindrule 'DIEMSV.DIEM'   -- HUY RANG BUOC
...
SELECT DISTINCT SINHVIEN.MASV,TENSV,SINHVIEN.DIACHI
FROM SINHVIEN,DIEMSV,DETAI,GIANGVIEN
WHERE SINHVIEN.MASV = DIEMSV.MASV
...
-- TRONG BANG DIEMSV BIET SINHVIEN CUA 1 DE TAI BANG TONG SO SINH VIEN TRONG BANG DIEM VOI MOI DE TAI
CREATE TRIGGER TG1 ON DIEMSV
AFTER INSERT,UPDATE
AS
	BEGIN
		UPDATE DETAI
		SET SOSV = (SELECT COUNT(DISTINCT DIEMSV.MASV) FROM DETAI,DIEMSV
					WHERE DIEMSV.MADT = DETAI.MADT
					GROUP BY DIEMSV.MADT)
	END
